...
static const struct tmffw_init_info tm_wheels[] = {
	...
	{ 0xb65e, FFW_UNKNOWN, "TRS Racing wheel", EFFECT_NONE },
...
static int tmffw_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	struct tm_wheel_drvdata *tm_wheel = NULL;
	enum legacy_ff_effect ff_effect;
	int i, ret = 0;
	...
	tm_wheel = devm_kzalloc(&hdev->dev, sizeof(struct tm_wheel_drvdata), GFP_KERNEL);
	if (!tm_wheel) {
		ret = -ENOMEM;
		goto err_stop;
	}
	...
	dev_set_drvdata(&tm_wheel->usb_dev->dev, tm_wheel);
	...
	case FFW_LEGACY:
		...
		break;
	case FFW_SETUP:
		ret = tmffw_init_probe(tm_wheel);
		if (ret)
			goto err_stop;
		break;
	case FFW_T300:
		dev_info(&hdev->dev, "PID %x is a FFW_T300 family device\n", id->product);
		break;
	case FFW_T150:
		dev_info(&hdev->dev, "PID %x is a FFW_T150 family device\n", id->product);
		break;
	case FFW_UNKNOWN:
		ret = -ENODEV;
		goto err_stop;
...
static const struct hid_device_id tmffw_devices[] = {
	{ HID_USB_DEVICE(USB_VENDOR_ID_THRUSTMASTER, 0xb300) }, /* Unknown */
	...
	{ HID_USB_DEVICE(USB_VENDOR_ID_THRUSTMASTER, 0xb65d) }, /* GIP Setup mode */
