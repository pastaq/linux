...
static void usb_thrustmaster_change_handler(struct urb *urb)
{
...
static void usb_thrustmaster_remove(struct usb_interface *iface)
{
 ...
 */
static int usb_thrustmaster_probe(struct usb_interface *iface,
				  const struct usb_device_id *id)
{
	...
	usb_fill_control_urb(urb, tm_wheel->usb_dev,
			     ...
			     usb_thrustmaster_change_handler, &iface->dev);
...
static const struct usb_device_id usb_thrustmaster_devices[] = {
...
MODULE_DEVICE_TABLE(usb, usb_thrustmaster_devices);

static struct usb_driver usb_thrustmaster_driver = {
	.name = "usb-thrustmaster",
	.id_table = usb_thrustmaster_devices,
	.probe = usb_thrustmaster_probe,
	.disconnect = usb_thrustmaster_remove,
};

module_usb_driver(usb_thrustmaster_driver);
